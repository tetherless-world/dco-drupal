 ...
 */
function commons_dco_grid_row($element, $name, $class='', $width='', $extra='') {
  $output = '';
  $extra = ($extra) ? ' ' . $extra : '';
  if ($element) {
    if ($class == 'full-width') {
...
function commons_dco_preprocess_page(&$variables) {
 $variables['pre_preface_top'] = theme('grid_row', $variables['pre_preface_top'], 'pre_preface_top', 'nested');
  ...
  /************************************
   * pcw - May 31, 2013 - removed this since we only need the description if
   * we're on the group home page. And that page is controlled via views and
   * blocks, and will display the description.
   ************************************/
  /*
...
function commons_dco_preprocess_node(&$variables) {
